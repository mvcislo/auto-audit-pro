...
2. QUALITY & LINKS: Use Google Search to find current market pricing for PREMIUM parts (e.g., Bosch, Akebono, Moog). Avoid low-end "economy" parts. Cite a legitimate source (e.g., RockAuto, PartsAvatar) with a direct link [Label](URL).
3. Analyze Appraiser Notes for specific wear items (e.g., "tires low", "brakes pulsing").
4. If Appraiser says "CLEAN", assume zero mechanical repairs beyond any mandatory prep dictated by [DEALERSHIP RULES].
...
4. GROUNDING & LINKS: Ensure all findings are grounded in legitimate sources. Provide direct links [Label](URL) to the parts you find as alternatives.
